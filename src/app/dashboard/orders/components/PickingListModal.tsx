...
import { OrderWithDetails } from '../utils/orderTypes'
import { downloadPickingListCSV } from '../utils/pickingListExporter'

interface PickingListModalProps {
  orders: OrderWithDetails[]  // Changed to OrderWithDetails to access items
  isOpen: boolean
  onClose: () => void
...
export default function PickingListModal({
  ...
  const totalItemsToPick = itemsCount ?? orders.reduce((total, order) => {
    return total + order.items.reduce((sum, item) => sum + item.quantity, 0)
  }, 0)

  // Consolidate ACTUAL items from all orders
  ...
  // Calculate picked items quantity
  const pickedItemsQuantity = useMemo(() => {
    return consolidatedItems
      .filter(item => pickedItems.has(item.sku))
      .reduce((sum, item) => sum + item.totalQuantity, 0)
  }, [consolidatedItems, pickedItems])

  // Calculate remaining items to pick
  const remainingItemsToPick = useMemo(() => {
    return totalItemsToPick - pickedItemsQuantity
  }, [totalItemsToPick, pickedItemsQuantity])

  // Force checkboxes to update before printing
  useEffect(() => {
  ...
  const handleExport = () => {
    downloadPickingListCSV(orders, warehouseName)
  }
              ...
              <Dialog.Panel className="w-full max-w-7xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all print:transform-none print:shadow-none print:rounded-none print:max-w-none print-content">
                ...
                <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4 print:hidden">
                  <div>
                    ...
                    <div className="mt-1 text-sm text-gray-500">
                      {actualOrdersCount} {actualOrdersCount === 1 ? 'order' : 'orders'} • {remainingItemsToPick} {remainingItemsToPick === 1 ? 'item' : 'items'} to pick
                ...
                <div className="hidden print:block px-6 py-4 border-b border-gray-200">
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    Generated: {new Date().toLocaleString()} • {actualOrdersCount} orders • {remainingItemsToPick} items to pick
                    {pickedItemsQuantity > 0 && ` • ${pickedItemsQuantity} items picked`}
                  </p>
                </div>
                              ...
                              <div className="mb-3">
                                <div className="flex items-center justify-between">
                                  <div>
                                    ...
                                    <p className="text-xs text-gray-500 mt-0.5">{order.customerName} • {formatDate(order.orderDate)}</p>
                                  </div>
                ...
                <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 print:hidden">
                  <p className="text-xs text-gray-500 text-center">
                    Generated on {new Date().toLocaleString()} • Progress: {pickedItemsQuantity}/{totalItemsToPick} items • {pickedOrders.size}/{actualOrdersCount} orders
                  </p>
                </div>
              </Dialog.Panel>
            </Transition.Child>
